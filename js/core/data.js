...
function mapLoanRow(row) {
  ...
  function recomputeDerivedLoanFields() {
    ...
    var schedules = App.state.schedules || [];
...
function normalizeAllSchedules(todayStr) {
    if (!App.state || !Array.isArray(App.state.schedules)) return;
    var list = App.state.schedules;
    for (var i = 0; i < list.length; i++) {
      normalizeScheduleStatus(list[i], todayStr);
    }
  }
...
function computePortfolioSummary(scope, dateRange) {
    ...
    if (scope === 'claim') {
      ...
      var schedulesForClaim = (App.state && App.state.schedules) ? App.state.schedules.slice() : [];
    ...
    var schedules = (App.state && App.state.schedules) ? App.state.schedules.slice() : [];
    if (!schedules.length) {
      return {
        scope: scope,
        plannedAmount: 0,
...
App.data.getClaimSummary = function(dateRange){
  var claims = (App.state && App.state.claims)? App.state.claims.slice():[];
  var claimTotal = claims.reduce((s,c)=>s+(Number(c.amount)||0),0);
  var schedules = (App.state && App.state.schedules)? App.state.schedules.slice():[];
  var collected=0;
  schedules.forEach(function(sc){
