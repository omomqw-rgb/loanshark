(function (window) {
  ...
  function runShadowQA(source) {
    ...
    if (App.schedulesEngine && typeof App.schedulesEngine.getAll === 'function') {
      ...
      schedules = (App.data && App.data.schedules) || (App.state && App.state.schedules) || [];
    }
  ...
  App.cloudState.apply = function (snapshot) {
    ...
    var snapshotSchedules = Array.isArray(data.schedules) ? data.schedules : [];

    if (App.schedulesEngine && typeof App.schedulesEngine.fromSnapshot === 'function') {
      App.schedulesEngine.fromSnapshot(snapshotSchedules);
    }
